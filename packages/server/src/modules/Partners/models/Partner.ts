...
export class PartnerQueryBuilder<M extends Model, R = M[]> extends BaseQueryBuilder<M, R> {
  constructor(...args) {
    // @ts-ignore
    super(...args);
    this.onBuild((builder) => {
      if (builder.isFind() || builder.isDelete() || builder.isUpdate()) {
        builder.where('contact_service', 'partner');
      }
    });
  }
}

export class Partner extends TenantBaseModel {
  ...
  static QueryBuilder = PartnerQueryBuilder;
