...
import { uploadQBOFileMulterOptions } from './ImportMulter.utils';
...
export class ImportController {
  constructor(
    private readonly importResourceApp: ImportResourceApplication,
    private readonly qboImportService: QBOImportService,
  ) { }
  ...
  async getImportFileMeta(@Param('import_id') importId: string) {
	...
	@Post('/qbo')
	@HttpCode(200)
	@ApiOperation({ summary: 'Upload QBO/OFX bank file' })
	@ApiResponse({ status: 200, description: 'QBO file uploaded successfully' })
	@UseInterceptors(FileInterceptor('file', uploadQBOFileMulterOptions))
	async qboUpload(
  	@UploadedFile() file: Express.Multer.File,
  	@Body('params') rawParams?: string,
	) {
  	const params = defaultTo(parseJsonSafe(rawParams), {});
  	return this.qboImportService.import(file.filename, params);
	}
}
